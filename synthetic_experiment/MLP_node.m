...
bnet=mk_bnet(dag,ns,'discrete',2); 
seed=0; 
rand('state',seed); 


bnet.CPD{A}=root_CPD(bnet,A);
bnet.CPD{B}=mlp_CPD(bnet,B,2);
...
for i=1:n_sample
    samples([1 3],i)=num2cell(obs_values(:,i));
end 
...
%plot(LLtrace,'x-');
bnet2=learn_params_em(engine,samples);
...
% %plot(x,y,'o');
% hold on 
% plot(x2,y_pred,'r+','markers',10);
% plot(x2,(x2-5).^2,'bo');
% title("Input feautre: (x-5)^2") 
% xlabel('x') 
% ylabel('y') 
